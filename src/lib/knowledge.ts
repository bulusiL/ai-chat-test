/**
 * 知识库服务
 * 用于管理图书和小学生学习相关的知识
 * 
 * 注意：使用此服务需要配置 coze-coding-dev-sdk 的相关凭证
 */

// 是否启用 SDK（检查是否配置）
const isSDKConfigured = (): boolean => {
  // 检查是否有必要的环境变量配置
  // 如果没有配置，返回 false，禁用 SDK 功能
  return false; // 默认关闭，需要配置 SDK 后开启
};

/**
 * 知识文档接口
 */
export interface KnowledgeEntry {
  id?: string;
  title: string;
  content: string;
  category: 'book' | 'study' | 'other';
  tags?: string[];
}

/**
 * 搜索结果接口
 */
export interface KnowledgeSearchResult {
  content: string;
  score: number;
  docId?: string;
}

// 内置知识库（离线模式使用）- 面向中国中小学生
const offlineKnowledge: KnowledgeEntry[] = [
  // ========== 中国经典文学 ==========
  {
    title: '适合小学低年级（1-2年级）阅读的中文图书推荐',
    category: 'book',
    tags: ['低年级', '绘本', '拼音', '中国作家'],
    content: `小学低年级（1-2年级）推荐阅读书目：

【中国经典绘本】
1. 《猜猜我有多爱你》（中文版）- 表达亲情，温馨感人
2. 《团圆》- 余丽琼著，讲述中国家庭的温情故事
3. 《一园青菜成了精》- 周翔编绘，童谣绘本，趣味十足
4. 《荷花镇的早市》- 周翔著，展现中国传统文化
5. 《安的种子》- 王早早著，关于等待与成长的故事

【桥梁书（带拼音）】
1. 《小猪唏哩呼噜》- 孙幼军著，语言幽默，适合朗读
2. 《神奇校车》中文版 - 科学启蒙，故事有趣
3. 《大头儿子小头爸爸》- 郑春华著，家庭温馨故事
4. 《没头脑和不高兴》- 任溶溶著，经典童话

【传统文化启蒙】
1. 《三字经》注音版
2. 《弟子规》注音版
3. 《成语故事》注音版（中国少年儿童出版社）
4. 《中国神话故事》注音版

阅读建议：每天15-20分钟，家长陪伴阅读。`
  },
  {
    title: '适合小学中年级（3-4年级）阅读的中文图书推荐',
    category: 'book',
    tags: ['中年级', '童话', '科普', '中国作家'],
    content: `小学中年级（3-4年级）推荐阅读书目：

【中国儿童文学经典】
1. 《夏洛的网》中文版 - 关于友谊与生命的经典
2. 《草房子》- 曹文轩著，唯美成长小说
3. 《窗边的小豆豆》中文版 - 黑柳彻子著，教育主题
4. 《狼王梦》- 沈石溪著，动物小说经典
5. 《淘气包马小跳》系列 - 杨红樱著，校园生活故事
6. 《笑猫日记》系列 - 杨红樱著，温馨童话

【科普读物】
1. 《十万个为什么》（少年儿童出版社）
2. 《可怕的科学》中文版系列
3. 《森林报》中文版 - 比安基著
4. 《中国儿童百科全书》

【历史与传统文化】
1. 《吴姐姐讲历史故事》- 吴涵碧著
2. 《中国民间故事》
3. 《林汉达中国历史故事集》
4. 《写给儿童的中国历史》

阅读建议：培养独立阅读习惯，每周至少2-3本书。`
  },
  {
    title: '适合小学高年级（5-6年级）阅读的中文图书推荐',
    category: 'book',
    tags: ['高年级', '名著', '成长', '中国作家'],
    content: `小学高年级（5-6年级）推荐阅读书目：

【中国文学经典】
1. 《城南旧事》- 林海音著，童年回忆经典
2. 《青铜葵花》- 曹文轩著，唯美成长小说
3. 《根鸟》- 曹文轩著，追寻梦想的故事
4. 《今天我是升旗手》- 黄蓓佳著，成长励志
5. 《我要做好孩子》- 黄蓓佳著

【外国名著中文译本】
1. 《汤姆·索亚历险记》中文版 - 马克·吐温著
2. 《鲁滨逊漂流记》中文版 - 笛福著
3. 《昆虫记》中文版 - 法布尔著
4. 《海底两万里》中文版 - 凡尔纳著
5. 《小王子》中文版 - 圣埃克苏佩里著

【历史与传记】
1. 《写给儿童的中国历史》（全彩版）
2. 《名人传》中文版 - 罗曼·罗兰著
3. 《苏东坡传》青少年版 - 林语堂著

阅读建议：培养深度阅读习惯，可以尝试写读书笔记。`
  },
  {
    title: '适合初中生阅读的中文图书推荐',
    category: 'book',
    tags: ['初中', '名著', '中国文学', '世界名著'],
    content: `初中生推荐阅读书目：

【中国现当代文学】
1. 《朝花夕拾》- 鲁迅著（必读）
2. 《骆驼祥子》- 老舍著（必读）
3. 《呐喊》- 鲁迅著
4. 《边城》- 沈从文著
5. 《平凡的世界》- 路遥著（青少年版）
6. 《围城》- 钱钟书著

【中国古典文学】
1. 《西游记》- 吴承恩著（原著或青少年版）
2. 《水浒传》- 施耐庵著（青少年版）
3. 《三国演义》- 罗贯中著（青少年版）
4. 《红楼梦》青少年版 - 曹雪芹著
5. 《古文观止》选读

【世界名著中文译本】
1. 《钢铁是怎样炼成的》- 奥斯特洛夫斯基著
2. 《老人与海》- 海明威著
3. 《简·爱》- 夏洛蒂·勃朗特著
4. 《悲惨世界》青少年版 - 雨果著

阅读建议：结合课内学习，做读书笔记，培养文学素养。`
  },
  // ========== 学习方法 ==========
  {
    title: '小学生语文学习方法',
    category: 'study',
    tags: ['语文', '阅读', '写作', '学习技巧'],
    content: `小学语文学习方法指导：

【识字与写字】
- 每天坚持练字15分钟
- 注意笔画顺序和字形结构
- 使用田字格练习，注意书写姿势

【阅读理解】
- 每天阅读30分钟以上
- 遇到不认识的字要查字典
- 读完后能说出主要内容
- 学会做简单的读书笔记

【写作技巧】
- 低年级：看图写话，从简单句子开始
- 中年级：写日记，记录生活点滴
- 高年级：写读后感，尝试多种文体

【古诗词学习】
- 每周背诵2-3首古诗
- 理解诗意，不只是死记硬背
- 了解诗人和创作背景
- 推荐书目：《小学生必背古诗词75+80首》

家长建议：多鼓励，少批评，培养孩子对语文的兴趣。`
  },
  {
    title: '小学生数学学习方法',
    category: 'study',
    tags: ['数学', '计算', '思维', '学习技巧'],
    content: `小学数学学习方法指导：

【基础计算】
- 低年级：口算练习，每天10-20题
- 中年级：熟练掌握四则运算
- 高年级：分数、小数混合运算
- 推荐工具：《口算天天练》

【应用题技巧】
- 认真读题，理解题意
- 找出已知条件和问题
- 画图帮助理解
- 检验答案是否合理

【几何图形】
- 认识基本图形的特征
- 动手剪、拼、折，建立空间概念
- 学会计算周长和面积

【数学思维】
- 多角度思考问题
- 学会找规律
- 尝试一题多解

【错题整理】
- 准备错题本
- 分析错误原因
- 定期复习错题

家长建议：不要只关注分数，要关注孩子的思考过程。`
  },
  {
    title: '小学生英语学习方法',
    category: 'study',
    tags: ['英语', '单词', '口语', '学习技巧'],
    content: `小学英语学习方法指导：

【词汇积累】
- 低年级：每课单词要会读、会说
- 中年级：开始拼写单词
- 高年级：学习词组和固定搭配
- 方法：制作单词卡片，每天复习

【听力训练】
- 每天听英语音频10-15分钟
- 推荐材料：课本配套音频、英语儿歌
- 看简单的英文动画片（带中文字幕）

【口语练习】
- 跟读课文录音
- 和同学用英语对话
- 唱英文歌曲

【阅读入门】
- 从简单的绘本开始
- 推荐：《牛津树》分级读物
- 推荐：《RAZ》分级阅读

【书写规范】
- 注意字母大小写
- 规范书写格式
- 练习抄写单词和句子

家长建议：创造英语环境，让学习变得有趣，不要给孩子太大压力。`
  },
  {
    title: '中小学生学习习惯和时间管理',
    category: 'study',
    tags: ['习惯', '时间管理', '效率', '方法'],
    content: `中小学生学习习惯和时间管理：

【作息时间建议】
- 早上：6:30-7:00 起床
- 放学后：先休息30分钟，再开始学习
- 学习时间：1-2小时（根据年级调整）
- 晚上：21:00前睡觉，保证充足睡眠

【作业习惯】
- 作业前：整理书桌，准备文具
- 作业中：专注完成，不分心玩手机
- 作业后：自我检查，收拾书包
- 难题处理：先思考，再求助

【预习和复习】
- 预习：提前了解明天要学的内容
- 复习：当天学习的知识当天巩固
- 周末：整理本周所学内容

【目标设定】
- 设定短期目标（如本周目标）
- 设定长期目标（如期中考试目标）
- 目标要具体、可衡量

【劳逸结合】
- 学习45分钟休息10分钟
- 课余时间进行体育运动
- 培养一项兴趣爱好

家长建议：以身作则，和孩子一起制定计划并执行。`
  },
  {
    title: '中小学生常见学习问题及解决方案',
    category: 'study',
    tags: ['问题', '注意力', '拖延', '解决方法'],
    content: `中小学生常见学习问题及解决方案：

【注意力不集中】
原因：环境干扰、睡眠不足、学习内容太难
解决方法：
- 创造安静的学习环境
- 保证充足睡眠
- 将学习任务分解成小块
- 使用番茄工作法（25分钟专注+5分钟休息）

【作业拖延】
原因：畏难情绪、缺乏动力、没有时间观念
解决方法：
- 制定作业时间表
- 先完成简单任务建立信心
- 设置合理的奖励机制
- 和孩子一起分析拖延原因

【成绩起伏大】
原因：知识点掌握不牢固、考试焦虑
解决方法：
- 查漏补缺，巩固基础
- 分析错题，找到薄弱环节
- 教孩子考试技巧
- 减轻考试压力，多鼓励

【不愿阅读】
原因：没有阅读兴趣、书太难、缺乏引导
解决方法：
- 从孩子感兴趣的主题开始
- 选择适合年龄的中文图书
- 亲子共读，营造阅读氛围
- 定期带孩子去书店或图书馆

【粗心大意】
原因：做题过快、没有检查习惯
解决方法：
- 强调解题步骤的重要性
- 培养检查习惯
- 整理"粗心本"，定期回顾

家长建议：耐心引导，不要过度批评，多鼓励孩子进步。`
  },
  // ========== 必读书目 ==========
  {
    title: '教育部推荐小学生必读书目',
    category: 'book',
    tags: ['必读', '教育部', '书单', '官方推荐'],
    content: `教育部推荐小学生必读书目：

【1-2年级】
1. 《猜猜我有多爱你》
2. 《安徒生童话》
3. 《格林童话》
4. 《伊索寓言》
5. 《没头脑和不高兴》
6. 《小猪唏哩呼噜》

【3-4年级】
1. 《中国古代寓言》
2. 《中国神话故事》
3. 《成语故事》
4. 《夏洛的网》
5. 《长袜子皮皮》
6. 《小王子》

【5-6年级】
1. 《西游记》青少年版
2. 《三国演义》青少年版
3. 《鲁滨逊漂流记》
4. 《汤姆·索亚历险记》
5. 《昆虫记》
6. 《城南旧事》
7. 《草房子》

【古诗词】
- 小学生必背古诗词75首+80首（人民教育出版社）

说明：以上为教育部基础教育课程教材发展中心推荐书目。`
  }
];

/**
 * 搜索知识库（离线模式 - 使用内置知识）
 */
export async function searchKnowledge(
  query: string,
  options: {
    topK?: number;
    minScore?: number;
  } = {}
): Promise<KnowledgeSearchResult[]> {
  const { topK = 5, minScore = 0.3 } = options;
  
  // 如果 SDK 已配置，使用在线知识库
  if (isSDKConfigured()) {
    try {
      // 动态导入 SDK
      const { KnowledgeClient, Config } = await import('coze-coding-dev-sdk');
      const config = new Config();
      const client = new KnowledgeClient(config);
      
      const response = await client.search(query, ['books_and_education'], topK, minScore);
      
      if (response.code === 0 && response.chunks) {
        return response.chunks.map(chunk => ({
          content: chunk.content,
          score: chunk.score,
          docId: chunk.doc_id
        }));
      }
    } catch (error) {
      console.error('Online knowledge search failed:', error);
      // 降级到离线模式
    }
  }
  
  // 离线模式：简单的关键词匹配
  const results: KnowledgeSearchResult[] = [];
  const queryLower = query.toLowerCase();
  
  for (const entry of offlineKnowledge) {
    let score = 0;
    const contentLower = entry.content.toLowerCase();
    const titleLower = entry.title.toLowerCase();
    const tagsLower = (entry.tags || []).join(' ').toLowerCase();
    
    // 标题匹配（权重高）
    if (titleLower.includes(queryLower)) {
      score += 0.5;
    }
    
    // 标签匹配
    if (tagsLower.includes(queryLower)) {
      score += 0.3;
    }
    
    // 内容匹配
    if (contentLower.includes(queryLower)) {
      score += 0.2;
    }
    
    // 关键词分词匹配
    const keywords = queryLower.split(/\s+/);
    for (const keyword of keywords) {
      if (keyword.length > 1) {
        if (titleLower.includes(keyword)) score += 0.1;
        if (contentLower.includes(keyword)) score += 0.05;
        if (tagsLower.includes(keyword)) score += 0.1;
      }
    }
    
    if (score >= minScore) {
      results.push({
        content: `【${entry.title}】\n\n${entry.content}`,
        score: Math.min(score, 1)
      });
    }
  }
  
  // 按相关度排序，返回前 topK 个
  return results
    .sort((a, b) => b.score - a.score)
    .slice(0, topK);
}

/**
 * 构建带知识上下文的系统提示词
 */
export function buildKnowledgePrompt(
  knowledgeResults: KnowledgeSearchResult[],
  userQuestion: string
): string {
  let prompt = `你是一个专业的图书推荐和学习辅导助手，专门为中国中小学生和家长提供服务。

## 核心原则
1. 必须使用中文回答
2. 优先推荐中国作家的优秀作品
3. 推荐中文译本，使用中文书名
4. 不要编造书名和作者

`;

  if (knowledgeResults.length > 0) {
    prompt += `## 相关知识库内容\n\n`;
    knowledgeResults.forEach((result, index) => {
      prompt += `### 参考资料 ${index + 1}\n`;
      prompt += `${result.content}\n\n`;
    });
  }

  return prompt;
}

export const KnowledgeService = {
  searchKnowledge,
  buildKnowledgePrompt,
};
